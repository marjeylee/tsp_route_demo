...
from config.config import VAL_SIZE, GRAPH_SIZE
...
class RolloutBaseline(Baseline):
    ...
    def __init__(self, model, problem, epoch=0):
        super(Baseline, self).__init__()
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is not None:
            if len(dataset) != VAL_SIZE:
                ...
                dataset = None
            elif (dataset[0] if self.problem.NAME == 'tsp' else dataset[0]['loc']).size(0) != GRAPH_SIZE:
                print("Warning: not using saved baseline dataset since graph_size does not match")
                dataset = None

        if dataset is None:
            self.dataset = self.problem.make_dataset(
                size=GRAPH_SIZE, num_samples=VAL_SIZE, distribution=None)
        else:
            self.dataset = dataset
        print("Evaluating baseline model on evaluation dataset")
        self.bl_vals = rollout(self.model, self.dataset).cpu().numpy()
        self.mean = self.bl_vals.mean()
        self.epoch = epoch

    def update_model(self, model, epoch, dataset=None):
        self.model = copy.deepcopy(model)
        # Always generate baseline dataset when updating model to prevent overfitting to the baseline dataset
    ...
    def wrap_dataset(self, dataset):
        ...
        return BaselineDataset(dataset, rollout(self.model, dataset).view(-1, 1))
    ...
    def epoch_callback(self, model, epoch):
        ...
        candidate_vals = rollout(model, self.dataset).cpu().numpy()
