...
from config.config import EMBEDDING_DIM, HIDDEN_DIM, N_ENCODE_LAYERS, TANH_CLIPPING, MASK_INNER, MASK_LOGITS, N_HEADS
...
class AttentionModelFixed:
    ...
    def __init__(self, node_embeddings=None, context_node_projected=None, glimpse_key=None, glimpse_val=None,
                 logit_key=None):
...
class AttentionModel(nn.Module):
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        input_function = lambda input: self._inner(*input)
        cost_function = lambda input, pi: self.problem.get_costs(input[0], pi)
        encoder = (input, self.embedder(self._init_embed(input))[0])
        return sample_many(input_function, cost_function,
                           # Don't need embeddings as input to get_costs
                           encoder,  # Pack input with embeddings (additional input)
                           batch_rep, iter_rep)
    ...
    def _get_log_p(self, fixed, state, normalize=True):
        _step_content = self._get_parallel_step_context(fixed.node_embeddings, state)
        # Compute query = context node embedding
        _step_content = self.project_step_context(_step_content)
        query = fixed.context_node_projected + _step_content
    ...
    def _get_parallel_step_context(self, embeddings, state, from_depot=False):
        ...
        if num_steps == 1:  # We need to special case if we have only 1 step, may be the first or not
            if state.i.item() == 0:
                # First and only step, ignore prev_a (this is a placeholder) initial state
